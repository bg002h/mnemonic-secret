...
- **An empty `--passphrase` from a private channel is announced** (F-687b,
  ...
  stdin is empty; proceeding with the EMPTY passphrase` (or `from environment
  variable VAR`) — and `ms derive` proceeds with the no-passphrase wallet.
  Never refused. Byte-identical to mnemonic-toolkit.
- **A terminal gets a prompt** (F-687b): when the passphrase is read from
...
### Fixed
- **`ms derive --allow-argv-secret --passphrase "- "` is a usage error (exit
  64), as in mnemonic-toolkit** (F-691). The argv guard's flag-shape test
...
### Changed
- **BREAKING (behaviour): `ms derive --passphrase -` reads the passphrase from
  stdin, and `--passphrase @env:VAR` from the environment** (F-687, operator
  ruling 2026-09-25). Before, `-` was the literal one-character passphrase:
  with `TREZOR` piped on stdin, "abandon ×11 about" derived `66d564d1`, exit
  0, where `--passphrase-stdin` derives `b4e3f5ed`. `@env:VAR` was refused by
  the argv guard as material, or taken literally under `--allow-argv-secret`.
  ...
  - The rule is shared with mnemonic-toolkit and pinned by
    ...
    named `VAR` an error naming it, set-but-empty the empty passphrase; a
    literal verbatim.
  - A literal argv passphrase (`--allow-argv-secret --passphrase <p>`) derives
    as before and prints ONE stderr line, `warning: secret material on argv
    (--passphrase) — read it privately with --passphrase - or
    --passphrase-stdin (stdin), or --passphrase @env:VAR (environment
    variable)`, replacing the old `pipe via --passphrase-stdin …` line. No
    line for `-`, `@env:` or `--passphrase-stdin`; stdout is unchanged.
...
name, and the corpus SHA moves; per `RELEASE_PROCESS.md` item 1 the pre-1.0
breaking-change axis requires `0.X+1.0`.

### What's new
- `HashKind` (`Sha256`, `Hash256`, `Ripemd160`, `Hash160`), `DigestBytes`, and
  one digest function per kind. `HashKind::digest` is the single named dispatch;
  callers do not write their own match. All four miniscript hash fragments take
  a 32-byte preimage — only the digest width differs (32 or 20), so the preimage
  side of a plate is unchanged for every kind.
- Corpus `tests/vectors/hashlock-v0.8.json`, SHA-256
  `0a911f78f3cdc867dcc44483b7f4c0c1ac87b6d9b30b79f52094e8979bc3d8ce` (was
  `4f1819cdd0862b101afd48d0478e8f0b218f933dd3da449915fa3c5eaaba21d4`): each
  `derivation` row gains `*_h_hash256`, `*_h_ripemd160` and `*_h_hash160`,
  computed in `python3 hashlib` OUTSIDE this crate — a row generated by the code
  it tests is not a KAT. The `qr_text` array gains a `kind` column and three
  per-kind rows (7 -> 10). **The fork's
  `hashlock/testdata/hashlock-v0.8.provenance.json` must re-pin to this hash**
  (phase 4).

### Breaking
- `qr_text` gains a `kind` parameter:
  `qr_text(hardened: bool, kind: HashKind, phrase: &str)`. The plate's QR text
  gains a `hash: <kind>` line between `method:` and `phrase:`, so a plate read
  years later names the hash it commits to rather than leaving it to the md1
  card the operator was told to store SEPARATELY (SPEC_hashlock_kinds §13.1).
  The worst case moves 194 -> **210 bytes** (hardened + `ripemd160`, the longest
  token) and stays at **53 QR modules** — the envelope already reserved, so no
  plate layout changes.
- `digest` is renamed `digest_sha256`. No deprecated alias: every internal call
  site would become a deprecation warning under `-D warnings`, a required CI
  context — measured at 14 in `ms-codec` and 4 in `ms-cli` with
  `--all-targets`.

### Migration notes
- See MIGRATION.md v0.9 → v0.10.

## ms-cli [0.19.0] — 2026-09-15

### What's new
- **`ms hashlock --kind <KIND>`** (`sha256`, `hash256`, `ripemd160`, `hash160`).
  Omitted, every kind's digest is listed on stderr and stdout keeps the bare
  sha256 record — §13.4 forbids assuming sha256 in silence, and that listing is
  printed whether or not `--no-engraving-card` suppresses the card. Case is
  rejected, never folded.
- The record follows §6's producer rule: **bare** `hash:<hex>` for sha256,
  `hash:<kind>:<hex>` otherwise. Every existing payload is byte-identical.
- The card's `for md compose:` line names the chosen kind's operand
  (`ripemd160=<hex>`) — the only line keeping the digest function and `md
  compose`'s option name in agreement.
...
- `--json` gains **`kind`** always, and — when `--kind` was omitted —
  **`kind_specified: false`** and **`digests_by_kind`** (all four digests).
  §13.4's notice cannot ride on stderr here, because `--json
  --no-engraving-card` pins stderr to exactly the PrivateKeyMaterial advisory
  (§4.4, §11); it changes channel rather than being dropped, so a consumer
  reading `hash_operand` alone cannot mistake the sha256 default for a stated
  choice.

### Breaking
- `--json`: the key `sha256_operand` becomes **`hash_operand`**, and its value
  names the chosen kind (`ripemd160=<hex>`). The old name is wrong for three of
  the four kinds. It is a machine-readable contract, though no code in this
  constellation reads either key today.

### Migration notes
- `ms-cli`'s `ms-codec` requirement moves from `=0.9.0` to `=0.10.0` in the same
