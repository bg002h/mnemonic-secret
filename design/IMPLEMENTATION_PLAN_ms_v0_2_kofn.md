...
- [ ] **Step 3: implement** `pub fn encode_shares(tag: Tag, threshold: Threshold, n: usize, secret: &Payload) -> Result<Vec<String>>`:
  ...
  - Use `getrandom::getrandom` (add to ms-codec Cargo.toml if absent — check; `mk_codec` precedent). Zeroize the random/secret intermediates.
...
- [ ] **Step 3: implement** `pub fn combine_shares(shares: &[String]) -> Result<(Tag, Payload)>`:
  ...
  - route `secret` through the existing `discriminate(&secret)` → `(Tag, Payload)` (NO re-implemented prefix/length checks; `discriminate` strips the prefix + `validate()`s). NB: `discriminate`'s threshold-check (`envelope.rs:105`) — the recovered secret-at-S has threshold `k`; ensure `discriminate` on the RECOVERED secret does not re-reject on threshold≠0. **Decision: `combine_shares` reads the payload from the interpolated secret's `data()` + dispatches on the prefix byte directly (reuse `discriminate`'s prefix→Payload logic, but NOT its threshold gate)** — factor the prefix-dispatch out of `discriminate` if needed, or pass a flag. (Flag for R0: the threshold gate must not fire on the recovered secret.)
...
- [ ] **Step 2: run → FAIL.**
- [ ] **Step 3: implement** `cmd/split.rs` reusing `cmd/encode.rs`'s source parsing (`:59-67` phrase/hex → entropy + language → `Payload::Entr`/`Payload::Mnem` exactly as encode's AUTO-route): build the secret `Payload`, `ms_codec::encode_shares(Tag::ENTR, Threshold::new(k)?, n, &payload)`, print N shares (text: one per line + engraving grouping; `--json`: `{shares, k, n, id, kind, language?}`). Add `Split(cmd::split::SplitArgs)` to the enum + dispatch. **Output-class:** `emit_output_class_advisory(OutputClass::PrivateKeyMaterial, …)` (advisory.rs:37) — the share SET is secret-equivalent. Wrap entropy in `Zeroizing`.
...
- [ ] **Step 3: implement.** In `analyze()` (`:49`): BEFORE rule 3, add a share branch — if `report.threshold ∈ 2..=9` → it's a share: `would_decode` semantics become "would combine (needs k)"; do NOT push `threshold-not-zero`/length failures. Surface `kind: share` + threshold/id/index in `emit_text`/`emit_json` (extend `InspectReport`/the report structs if the fields aren't already present — `report.threshold` exists per `:58`).
- [ ] **Step 4: run → PASS.** Commit.
...
- [ ] **Step 1: failing tests:** `mnemonic ms-shares split --phrase "<ja>" -k 2 -n 3` → 3 mnem-shares; `mnemonic ms-shares combine <s1> <s2> --to ms1` → recovered ms1; `--to phrase` → ja phrase; `combine --to ms1` piped into `bundle --slot @0.ms1=<recovered>` → a valid bundle (composition). entr + mnem.
- [ ] **Step 2: run → FAIL.**
...
- [ ] **Step 1:** `crates/ms-cli/tests/decode_rejects_threshold_not_zero.rs` currently asserts a threshold=2 string → `ThresholdNotZero` (text `:25` "threshold"; json `:46` kind `"ThresholdNotZero"`). The new behavior: → `IsShareNotSingleString`. **Re-spec** the test (rename to `decode_routes_share_to_is_share_not_single_string` or update assertions) — prove the NEW behavior is correct (a threshold=2 string IS a share). **Grep ALL tests** (`grep -rn 'ThresholdNotZero\|threshold' crates/*/tests/`) for other assertions on the old reject; update each.
