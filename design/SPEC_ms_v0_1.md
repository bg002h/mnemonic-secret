...
3. Has threshold ≠ `0` (`Error::ThresholdNotZero`). v0.1 is single-string only.
4. Has share-index ≠ `s` (`Error::ShareIndexNotSecret`). Required for threshold = 0 per BIP-93 §"Specification".
...
Restated verbatim from `MIGRATION.md` for SPEC-locality. v0.2 will add K-of-N share encoding for `entr` while preserving forward-readability of v0.1 entr strings. Four invariants:
...
The `seed` / `xprv` payload framing — out of scope for v0.1 because they don't fit BIP-93's brackets — is a *separate* design problem for v0.2+ and not subject to this migration contract. The v0.2 plan when written must specify whether `seed`/`xprv` ride a different sub-format (different HRP, forked codex32 with widened brackets, or an entirely new BCH design) and how they coexist on the wire with v0.2's entr-K-of-N strings. **If a future v0.2+ payload kind ends up sharing HRP `ms` (e.g., a widened-bracket BIP-93 variant), it MUST claim a distinct prefix-byte value (`0x02`, `0x03`, …) and the grouping-invariant text in #2 above MUST be updated to dispatch that prefix to its own kind-specific path.** v0.2's grouping-by-id semantics are scoped to `entr` shares only and are not implicitly portable to other kinds. The v0.2 SPEC MUST also include a small registry table of allocated prefix-byte values (`0x00 = v0.1-entr`, `0x01 = v0.2-entr-share`, `0x02..0xFF = unallocated, claim-via-PR`) so two future v0.2+ kinds cannot race for the same byte.

All four invariants are also captured in `MIGRATION.md` and (for invariant 1, the user-visible part) in the source comments of `crates/ms-codec/src/envelope.rs`.
...
|---|---|---|
| K-of-N share encoding for `entr` | v0.2 deliverable; designed-up-front via the prefix-byte migration contract (§5) | ms-codec v0.2 |
