...
- **✅ RED FIXED 2026-08-15; the guard is now DORMANT, which is the residual item.** The repair was not the tempting one. Loosening the exit-code assertion to accept 4 would have made the comparison green — and it would have been comparing **ms-codec against itself**: `mnemonic-toolkit` depends on `ms-codec = "0.7"` and its repair path calls `ms_codec::decode_with_correction` (`mnemonic-toolkit/src/repair.rs:8`). This test's own header always said a post-B.7 parity check is "tautological (same code, twice)" and that a version mismatch must SKIP; the code contradicted its header and only logged. The header was right. The version gate now exists (`PARITY_TOOLKIT_VERSION`), and it skips with a message saying the guard is **DORMANT, not passing** — so a green suite cannot be misread as "parity verified". `cargo test` exit code is now 0; 409 passed, 0 failed.
- **Residual:** the BCH port has no live independent cross-check any more, because the only other implementation became this one. That is a real coverage loss and no test can paper over it. **Options:** (a) accept it — the port has its own vectors, adversarial and property tests; (b) pin a v0.22.1 toolkit binary as a build artifact so the guard can actually run; (c) cross-check against a third-party codex32 implementation instead, which is the only option that restores genuine independence.
- **2026-08-19 — the file was measured to assert NOTHING, and half of that is now fixed.** The constellation journey recon flagged this as one of four skip-passes-instead-of-fails instances; reproducing it showed something the entry above did not record: the version gate returns **before** the test's step 1, which asserts that **ms-codec itself** corrects a corrupted ms1 and reports exactly one correction. That assertion never needed the toolkit, so real codec coverage was hostage to a binary that will never be present, and the whole file printed `ok` having executed no assertion at all. Demonstrated rather than argued: with `decode_with_correction` mutated to report **no** corrections, and again to report the **wrong position**, `parity_smoke_ms_against_toolkit_v0_22_1` **passed both times**. The toolkit-independent half now runs unconditionally and is widened from one corruption to a battery of six positions/masks, each asserting the reported position, `was`, `now`, and that applying the correction **reconstructs the original codeword** — which the gated version could only check by shelling out to the toolkit. A companion test pins that a clean string reports no correction, so a decoder that always "corrected" something cannot satisfy the battery. All three mutations above are now caught.
- **The residual is UNCHANGED and is still (a)/(b)/(c) above:** there is still no independent implementation to cross-check the BCH port against, and nothing here creates one. The cross-validation is left dormant rather than deleted or forced green.
- **Status:** OPEN (residual only; the red is gone). **Tier:** `test-infra`.
...
- **Status:** ✓ **RESOLVED — shipped-but-unflipped, confirmed 2026-08-03 by the maturity-program reconcile.** The hardcoded `assert_eq!(pin.page_count, 1, …)` is GONE from `crates/ms-cli/src/mlock.rs` and from the byte-synced toolkit copy. It was replaced by the correct contract — `let expected = round_to_pages(v.as_ptr() as usize, v.len(), page_size()).1;` then `assert_eq!(pin.page_count, expected, …)` plus `assert!((1..=2).contains(&pin.page_count), …)` — whose in-file comment cites this FOLLOWUP by name ("g4a FOLLOWUP"). That is the recorded fix intent (assert the real contract, not a hardcoded 1), and it is stronger than the filed minimal fix (delete the line), since it pins the round_to_pages relationship rather than dropping the assertion. Verified present in BOTH repos, so the g6 byte-sync invariant is intact and NO ms-cli tag is required. Status flipped late — the deliverable shipped without the same-commit flip this registry's discipline calls for.
- **Tier:** `cross-repo`
