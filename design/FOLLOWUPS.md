...
- **Status:** `open` (catalog). Port `mnemonic-toolkit:ci/repro/vendor-freshness.sh` + `.github/workflows/vendor-freshness.yml` verbatim — **TWO-block** form (`ms-codec` is fork-free / no miniscript `[patch]`, so `MINISCRIPT_REV=""`). **Tier:** `ci`. **Companion:** `mnemonic-toolkit` `design/FOLLOWUPS.md::vendor-freshness-pr-gate` (RESOLVED there 2026-06-26) + `docs/verify-reproducibility.md`.
