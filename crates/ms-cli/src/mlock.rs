...
//!
//! INLINE COPY of `mnemonic-toolkit/crates/mnemonic-toolkit/src/mlock.rs`
//! per Cycle B SPEC §5 + §6 G6 (no shared mlock crate; constellation stays
//! at 4 crates; CI invariant test prevents drift). When updating, mirror
//! the change in BOTH repos and push within the same PE session.
//!
//! Cycle B Phase 2 (mlock infrastructure). See
//! `mnemonic-toolkit/design/SPEC_secret_memory_hygiene_v0_9_B.md` §4 P2 for
//! the locked Fix-B design: slice-fn primitive only (no wrapper type).
//!
//! Module surface:
